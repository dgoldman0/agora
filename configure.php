...
if (!mysqli_connect_errno($con))
{
	...
	$result = $con->query("select Count(*) as tcnt from information_schema.tables where table_type='BASE TABLE' and table_schema='".DB_NAME."'");
		...
		if ($row['tcnt'] == 0)
		{
			...
			} else
			{
				...
				if ($password == $_POST["password2"])
				{
					...
					$base = "id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, created_on TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_on TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP";
					// Add a stylized name column and set naming requirements similar to shop names
					if ($con->error == "") $con->query("CREATE TABLE users (id INT(11) unsigned NOT NULL auto_increment, created_on TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_on TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, username varchar (50) NOT NULL DEFAULT '', password VARCHAR (255) NOT NULL, salt VARCHAR(255), user_role INT(11) unsigned NOT NULL default '1', email VARCHAR (50) NOT NULL DEFAULT '', first VARCHAR(50) NOT NULL DEFAULT '', last VARCHAR(50) NOT NULL DEFAULT '', PRIMARY KEY (id), UNIQUE KEY(username)) ENGINE=InnoDB;");
					...
					// Shop tables
					if ($con->error == "") $con->query("CREATE TABLE shop_types (id SMALLINT(5) UNSIGNED NOT NULL AUTO_INCREMENT, title VARCHAR(255) NOT NULL, description LONGBLOB NOT NULL, PRIMARY KEY(id)) ENGINE=InnoDB;");
					// Need on delete change to different shop type?
					if ($con->error == "") $con->query("CREATE TABLE shops (id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, name VARCHAR(255) NOT NULL DEFAULT '', stylized VARCHAR(255) NOT NULL, short_desc VARCHAR(156) NOT NULL DEFAULT '', url VARCHAR(255) NOT NULL, shop_type SMALLINT(5) UNSIGNED NOT NULL DEFAULT 0, PRIMARY KEY(id), FOREIGN KEY(shop_type) REFERENCES shop_types(id), UNIQUE KEY(name), UNIQUE KEY(url)) ENGINE=InnoDB;");
					...
					if ($con->error == "") $con->query("CREATE TABLE item_images ($base, item_id INT(11) UNSIGNED NOT NULL, title VARCHAR(60) NOT NULL DEFAULT '', alt_text VARCHAR(60) NOT NULL DEFAULT '', long_desc VARCHAR(255) NOT NULL, url VARCHAR(255) NOT NULL, PRIMARY KEY(id), FOREIGN KEY(item_id) REFERENCES items(id) ON DELETE CASCADE) ENGINE=InnoDB;");
					
					// Default shop id is 0 ie marketwide
					// Should really change the name of item_price_categories to just price_categories...
					if ($con->error == "") $con->query("CREATE TABLE item_price_categories (id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, shop_id INT(11) UNSIGNED NOT NULL, description VARCHAR(156) NOT NULL, PRIMARY KEY(id), FOREIGN KEY(shop_id) REFERENCES shops(id) ON DELETE CASCADE) ENGINE=InnoDB;");
					if ($con->error == "") $con->query("CREATE TABLE item_prices (id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, item_id INT(11) UNSIGNED NOT NULL, price_category INT(11) UNSIGNED NOT NULL, currency INT(11) UNSIGNED NOT NULL, value DECIMAL(10,3) NOT NULL DEFAULT 0, PRIMARY KEY(id), FOREIGN KEY(item_id) REFERENCES items(id) ON DELETE CASCADE, FOREIGN KEY(price_category) REFERENCES item_price_categories(id) ON DELETE CASCADE) ENGINE=InnoDB;");
					...
					if ($con->error == "") $con->query("CREATE TABLE shopping_bags (id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, created_on TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_on TIMESTAMP DEFAULT CURRENT_TIMESTAMP on update CURRENT_TIMESTAMP, cart_id INT(11) UNSIGNED NOT NULL, shop_id INT(11) UNSIGNED NOT NULL, active TINYINT(1) UNSIGNED NOT NULL DEFAULT 0, PRIMARY KEY(id), FOREIGN KEY(cart_id) REFERENCES shopping_carts(id) ON DELETE CASCADE, FOREIGN KEY(shop_id) REFERENCES shops(id) ON DELETE CASCADE) ENGINE=InnoDB;");
					if ($con->error == "") $con->query("CREATE TABLE bag_items (id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, created_on TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_on TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, bag_id INT(11) UNSIGNED NOT NULL, item_id INT(11) UNSIGNED NOT NULL, cnt INT(11) UNSIGNED NOT NULL DEFAULT 1, PRIMARY KEY(id), UNIQUE KEY(bag_id, item_id), FOREIGN KEY(bag_id) REFERENCES shopping_bags(id) ON DELETE CASCADE, FOREIGN KEY(item_id) REFERENCES items(id) ON DELETE CASCADE) ENGINE=InnoDB;");
					
					// Payment tables
					if ($con->error == "") $con->query("CREATE TABLE payment_info (id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, name VARCHAR(255) NOT NULL DEFAULT '', user_id INT(11) UNSIGNED NOT NULL, payment_type SMALLINT(5) UNSIGNED NOT NULL, PRIMARY KEY(id), FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE);");
					if ($con->error == "") $con->query("CREATE TABLE credit_cards (id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, piid INT(11) UNSIGNED NOT NULL, PRIMARY KEY(id), uri VARCHAR(255) NOT NULL, FOREIGN KEY(piid) REFERENCES payment_info(id) ON DELETE CASCADE);");

					// Order & Invoice tables
					if ($con->error == "") $con->query("CREATE TABLE orders (id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, user_id INT(11) UNSIGNED NOT NULL, placed_on TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY(id), FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE) ENGINE=InnoDB;");
					if ($con->error == "") $con->query("CREATE TABLE invoices (id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, order_id INT(11) UNSIGNED NOT NULL, shop_id INT(11) UNSIGNED NOT NULL, payment_method INT(11) UNSIGNED NOT NULL, PRIMARY KEY(id), FOREIGN KEY(order_id) REFERENCES orders(id) ON DELETE CASCADE, FOREIGN KEY(shop_id) REFERENCES shops(id) ON DELETE CASCADE, FOREIGN KEY(payment_method) REFERENCES payment_info(id)) ENGINE=InnoDB;");
					if ($con->error == "") $con->query("CREATE TABLE invoice_items (id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, invoice_id INT(11) UNSIGNED NOT NULL, item_id INT(11) UNSIGNED NOT NULL, cnt INT(11) UNSIGNED NOT NULL DEFAULT 1, price DECIMAL(10, 3) NOT NULL DEFAULT 0, PRIMARY KEY(id), UNIQUE KEY(invoice_id, item_id), FOREIGN KEY(invoice_id) REFERENCES invoices(id) ON DELETE CASCADE, FOREIGN KEY(item_id) REFERENCES items(id) ON DELETE CASCADE) ENGINE=InnoDB;");
					// Product Reviews
					
					// Modules
					if ($con->error == "") $con->query("CREATE TABLE modules (location VARCHAR(255) NOT NULL, active TINYINT(1) NOT NULL DEFAULT 0, PRIMARY KEY(location)) ENGINE=InnoDB");
					// Templates
					
					// Custom content
					// Need to add shop_id for media
					if ($con->error == "") $con->query("CREATE TABLE media (id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, uploaded_on TIMESTAMP DEFAULT CURRENT_TIMESTAMP, name VARCHAR(255) NOT NULL, type SMALLINT(5), data LONGBLOB NOT NULL, alt_text VARCHAR(255), long_desc TEXT NOT NULL, PRIMARY KEY(id)) ENGINE=InnoDB;");
					// Do some kind of on delete set default thing here
					if ($con->error == "") $con->query("CREATE TABLE badges (id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, medium_id INT(11) UNSIGNED NOT NULL, shop_id INT(11) UNSIGNED NOT NULL, name VARCHAR(255) NOT NULL, description LONGBLOB NOT NULL, free TINYINT(1) NOT NULL DEFAULT 1, PRIMARY KEY(id), FOREIGN KEY(medium_id) REFERENCES media(id), FOREIGN KEY(shop_id) REFERENCES shops(id)) ENGINE=InnoDB;");
					if ($con->error == "") $con->query("CREATE TABLE user_badges (id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, user_id INT(11) UNSIGNED NOT NULL, badge_id INT(11) UNSIGNED NOT NULL, PRIMARY KEY(id), FOREIGN KEY(user_id) REFERENCES users(id), FOREIGN KEY(badge_id) REFERENCES badges(id)) ENGINE=InnoDB;");
					if ($con->error == "") $con->query("CREATE TABLE pages (id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, title VARCHAR(255) NOT NULL, perma VARCHAR(255) NOT NULL, shop_id INT(11) UNSIGNED NOT NULL, tstamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP, content MEDIUMBLOB NOT NULL, type SMALLINT(5) UNSIGNED NOT NULL DEFAULT 1, PRIMARY KEY(id, created_on), FOREIGN KEY(shop_id) REFERENCES shops(id) ON DELETE CASCADE) ENGINE=InnoDB;");
					if ($con->error == "") $con->query("CREATE TABLE page_authors ($base, page_id INT(11) UNSIGNED NOT NULL, author_id INT(11) UNSIGNED NOT NULL, coauthor TINYINT NOT NULL DEFAULT 0, PRIMARY KEY(id), FOREIGN KEY(page_id) REFERENCES pages(id) ON DELETE CASCADE, FOREIGN KEY(author_id) REFERENCES users(id) ON DELETE CASCADE) ENGINE=InnoDB;");
					if ($con->error == "") $con->query("CREATE TABLE boards (id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, created_on TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_on TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, shop_id INT(11) UNSIGNED NOT NULL, title VARCHAR(60) NOT NULL, open TINYINT(1) NOT NULL 0, abstract_only TINYINT(1) NOT NULL DEFAULT 0, PRIMARY KEY(id), FOREIGN KEY(shop_id) REFERENCES shops(id) ON DELETE CASCADE) ENGINE=InnoDB;");
					...
					if ($con->error == "") $con->query("CREATE TABLE restaurant_menus (id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, title VARCHAR(255) NOT NULL, description LONGBLOB NOT NULL, shop_id INT(11) UNSIGNED NOT NULL, created_on TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY(id), FOREIGN KEY(shop_id) REFERENCES shops(id) ON DELETE CASCADE) ENGINE=InnoDB;");
					if ($con->error == "") $con->query("CREATE TABLE restaurant_menu_items (id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, menu_id INT(11) UNSIGNED NOT NULL, PRIMARY KEY(id), FOREIGN KEY(id) REFERENCES items(id), FOREIGN KEY(menu_id) REFERENCES restaurant_menus(id)) ENGINE=InnoDB;");

					// Social Networking
					if ($con->error == "") $con->query("CREATE TABLE friends ($base, user_id INT(11) UNSIGNED NOT NULL, friend_id INT(11) UNSIGNED NOT NULL, PRIMARY KEY(id), FOREIGN KEY(friend_id) REFERENCES users(id) ON DELETE CASCADE) ENGINE=InnoDB;");
					if ($con->error == "") $con->query("CREATE TABLE invites ($base, invited_by INT(11) UNSIGNED NOT NULL, expires TIMESTAMP NOT NULL, PRIMARY KEY(id), FOREIGN KEY(invited_by) REFERENCES users(id) ON DELETE CASCADE) ENGINE=InnoDB;");
					...
					if ($con->error == "") $con->query("CREATE EVENT clean ON SCHEDULE EVERY 1 HOUR DO DELETE FROM sessions WHERE expires < UNIX_TIMESTAMP();");
//					if ($con->error == "") $con->query("CREATE TRIGGER rmi_add AFTER INSERT on items BEGIN IF (SELECT shop_type FROM shops WHERE id=NEW.shop_id)=1 THEN INSERT INTO restaurant_menu_items  END IF; END; delimiter ;
